...
-- DuckDB: generate_series returns a list; unnest explodes it to rows.
select
    unnest(
        generate_series('2020-01-01'::date, '2030-12-31'::date, interval '1 day')
    )::date as date_day
{% else %}
-- Databricks/Spark: sequence + explode
select
    explode(sequence(to_date('2020-01-01'), to_date('2030-12-31'), interval 1 day)) as date_day
{% endif %}
