# run_silver.ps1 - PowerShell counterpart of scripts/bash/run_silver.sh
# Builds and tests the silver dbt models; same behavior and log output.

Set-StrictMode -Version Latest

$RepoRoot = (Resolve-Path (Join-Path $PSScriptRoot '..\..')).Path
$LogFile = Join-Path $RepoRoot "logs\run_silver_$(Get-Date -Format 'yyyy-MM-dd').log"
...
function Invoke-Logged([string]$FileName, [string[]]$Arguments) {
    ...
    if ($LASTEXITCODE -ne 0) {
        Log "[run_silver] ERROR exit=$LASTEXITCODE"
        exit $LASTEXITCODE
    }
}

$DbtProfilesDir = if ($env:DBT_PROFILES_DIR) { $env:DBT_PROFILES_DIR } else { Join-Path $RepoRoot 'dbt' }
$DbtSelect = if ($env:DBT_SELECT) { $env:DBT_SELECT } else { 'tag:silver' }

Log "[run_silver] start profiles=$DbtProfilesDir select=$DbtSelect"
